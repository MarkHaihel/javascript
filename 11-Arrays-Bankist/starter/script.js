...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  dogs.forEach(function (year, i) {
    if (year >= 3) {
      console.log(`Dog number ${i + 1} is an adult, and is ${year} old`);
    } else {
...
const calcAverageHumanAge = function (ages) {
  ...
  // const agerage = adults.reduce((acc, age) => acc + age, 0) / adults.length;
  const agerage = adults.reduce(
    (acc, age, i, arr) => acc + age / adults.length,
    0
  );

  //   2 3. (2 + 3) / 2 = 2.5 === 2/2 + 2/3
...
let jesAccount;
for (const acc of accounts) if (acc.owner === 'Jessica Davis') jesAccount = acc;
console.log(jesAccount);
*/
...
/*
// Strings
const owners = ['Mark', 'Zach', 'Adam', 'Martha'];
...
console.log(convertTitleCase('and here is another titltle with EXAMPLE'));
*/
...
// 1.
dogs.forEach(dog => (dog.recFood = dog.weight ** 0.75 * 26));
console.log(dogs);
...
// 5.
console.log('Has EXACLY recomended'.padStart(30, '-').padEnd(40, '-'));
console.log(dogs.some(dog => dog.recFood === dog.curFood));

// 6.
console.log('Has OKAY recomended'.padStart(30, '-').padEnd(40, '-'));
